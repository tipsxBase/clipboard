...
const confirmSelection = async () => {
  if (!selection.value || !bgCanvas.value) return;

  const tempCanvas = document.createElement("canvas");
  tempCanvas.width = selection.value.w;
  tempCanvas.height = selection.value.h;
...
<template>
  <div
    ...
    :style="{ cursor: isReady ? cursorStyle : 'wait' }"
  ...
  >
    ...
    >
      ...
      >
        ...
        <div class="flex items-center border-r border-gray-600 px-1">
          <button
            class="p-2 hover:bg-white/10 transition-colors rounded opacity-50 cursor-not-allowed"
            title="撤销"
            disabled
          >
