    ...
    const handleMouseMove = (e: MouseEvent) => {
        ...
        if (isResizing.value && dragStartSelection.value && resizeHandle.value) {
            ...
            switch (resizeHandle.value) {
                case 'nw':
                    newX = sx + dx; newY = sy + dy; newW = sw - dx; newH = sh - dy;
                    break;
                case 'ne':
                    newY = sy + dy; newW = sw + dx; newH = sh - dy;
                    break;
                case 'sw':
                    newX = sx + dx; newW = sw - dx; newH = sh + dy;
                    break;
                case 'se':
                    newW = sw + dx; newH = sh + dy;
                    break;
                case 'n':
                    newY = sy + dy; newH = sh - dy;
                    break;
                case 's':
                    newH = sh + dy;
                    break;
                case 'w':
                    newX = sx + dx; newW = sw - dx;
                    break;
                case 'e':
                    newW = sw + dx;
                    break;
    ...
    onMounted(async () => {
        try {
            ...
            const processCaptures = async (allCaptures: CaptureResult[]) => {
                ...
                const targetCapture = sId !== null
                    ? allCaptures.find((c) => c.id === sId)
                    : allCaptures[0];
